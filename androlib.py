    ...
    for div in divs:
        for d in div:
            if d.name == 'a' and d.has_key('href'):
                ...
                db.db_execute_g(db.sql_lib_lang_insert, (lang_href, lang_title, str(datetime.now())))


def home_read_main():
    global lib_host_http
    global lib_conn_http
    rows = db.db_get_g(db.sql_lib_lang_get, ())
    for row in rows:
        lang_href = row[0]
        lang_title = row[1]
        ...
        try:
            home_read(lang_href)
            db.db_execute_g(db.sql_lib_lang_update, (lang_href,))
...
def home_read(lang_href):
    ...
    for menu_f in menu_fa:
        if menu_f.has_key('href'):
            ...
            db.db_execute_g(db.sql_lib_lang_cate_insert, (lang_href, menu_href, menu_name, str(datetime.now())))
...
def home_read_max_main():
    global lib_host_http
    global lib_conn_http
    rows = db.db_get_g(db.sql_lib_lang_cate_get, ())
    for row in rows:
        lang_href = row[0]
        cate_path = row[1]
        cate_title = row[2]
...
def p_num_read(lang_href, menu_href, cate_href, cate_name):
    ...
    for i in range(0, last_final+1):
        db.db_execute_g(db.sql_lib_lang_cate_read_insert, (lang_href, cate_href, i, str(datetime.now())))
    db.db_execute_g(db.sql_lib_lang_cate_update, (last_final, lang_href, menu_href)) # becareful the menu_href here is different to cate_href
    print last_ps, last_next, last_final

def cate_read_main():
    global lib_host_http
    global lib_conn_http
    rows = db.db_get_g(db.sql_lib_lang_cate_read_get, ())
    for row in rows:
        lang_href = row[0]
        cate_path = row[1]
        cate_param = row[2]
...
def cate_read(lang_href, cate_path, cate_param):
    ...
    for a_f in a_fa:
        if a_f.has_key('href'):
            ...
            db.db_execute_g(db.sql_lib_lang_cate_link_read_insert, (lang_href, cate_path, cate_param, link_href, link_name, link_id))
    db.db_execute_g(db.sql_lib_lang_cate_read_update, (lang_href, cate_path, cate_param))

def cate_link_read_main():
    global lib_host_http
    global lib_conn_http
    rows = db.db_get_g(db.sql_lib_lang_cate_link_read_get, ())
    for row in rows:
        lang_href = row[0]
        cate_path = row[1]
        cate_param = row[2]
